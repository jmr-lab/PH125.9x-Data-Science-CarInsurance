...
\[
f_hat(x) = b_0 * Date_last_renewal^4 + b_1 * R_Claims_history^9 + b_2 * Driving_age^8
\]
...
If we take into consideration the Status and the Value_vehicle variables, we get the following model :
...
Incorporating the biases to our model doesn't change the relative error, but increase the RMSE and the Accuracy :
...
The RMSE rose from 470 to 476, while the relative error remained within 1% of the target. Notably, the accuracy improved significantly from 15% to 24.6%.
...
```{r, eval=FALSE}
# floor negatives
valid_subset$lm_pr_adj <- pmax(valid_subset$lm_pr_adj, 0)
```

We can calculate the RMSE, relative error and accuracy of the predictions for each offset. We will try for all offset values between 1000 and 60000, and for low values we will also try 0.001, 0.01, 0.1, 1, 10, 100 :

```{r, echo=FALSE, out.width='100%', fig.align="center", fig.cap = "Offset for log10 transformation", fig.height=5}
offset_results_plot
```

As we can see the RMSE increases with the offset, with a decrease at low values. The relative error increases to be close to zero when the offset is about 50,000. The accuracy decreases with the offset.

\newpage

If we want to get the relative error under the 1% value, and at the same time keep the lowest RMSE and the highest accuracy, we need to take an offset of approximately 47,000. The results will then be :
...
The "PR + log10 transformation" method not only minimizes the RMSE to 470.66, signifying precision in predictions, but also achieves a low relative error close to zero, indicating that the model's predictions are nearly perfect, with minimal deviation from true values. Furthermore, the accuracy of 0.491 indicates that a significant proportion of the model's predictions are correct, demonstrating robust performance in estimating costs. This model stands out as the most reliable approach among those tested, providing confidence in its ability to deliver accurate cost estimations while effectively handling potential variability in the data.
