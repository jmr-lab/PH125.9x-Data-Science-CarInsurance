...
Here's a simple schema illustrating a DNN where each neuron is represented by a circle, each synapse by an arrow, and the neurons of the input layers are in red, while the (only) neuron of the output layer is in green :

```{r, echo=FALSE, message=FALSE, out.width='50%', fig.align="center", fig.cap = "Example of a Deep Neural Network"}
knitr::include_graphics("deep-neural-network.png")
```
...
In contrast to simpler models, such as linear and polynomial regression, the DNN fails to demonstrate superior predictive accuracy.
...
This technique is particularly useful for modeling data when the relationship between variables is not adequately captured by a straight line, enabling better predictions and insights. Polynomial regression incorporates powers of the independent variable(s) (e.g., x^2, x^3) into the model, which allows for bending the curve to fit the data more accurately.
...
```{r, echo=FALSE, fig.align="center", fig.cap = "Total Cost per Week", fig.height=3}
weekly_summary_plot
```
...
We can further improve our model by remove negative predictions and at the same time trying to keep the same total cost. We can also try to use the best predictions from each model.
