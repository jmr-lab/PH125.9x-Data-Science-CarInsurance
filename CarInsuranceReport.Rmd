...
We saw earlier that the yearly cost is somewhat related to the number of claims submitted during the year, and that it also dicreases everery year. Let's see the monthly trend for the number of policies (renewal), cost and number of claims :
...
- **Sub-Train Set**: This smaller subset will be used for the actual training of the predictive model. Feature engineering will focus on elements such as customer demographics, driving history, and vehicle specifications that significantly influence premiums.

- **Sub-Valid Set**: This subset will be employed for validation purposes, allowing me to assess how well the model generalizes to unseen data. The **sub_valid_set** will serve as a crucial checkpoint to evaluate the model's performance and mitigate the risk of overfitting to the training data.

During the training phase, hyperparameters will be optimized to enhance predictive accuracy.

**Validation and Incremental Learning**

Upon completing the training phase, the model's performance will be evaluated using the **sub_valid_set**. The Root Mean Square Error (RMSE) metric will be the primary measure of model accuracy. This metric assists in determining how closely the predicted insurance costs align with the actual costs at the individual customer level.
...
The goal here is to achieve a low RMSE, indicating that the predictions are reliable for the end-users. If the validation results are satisfactory, I will consider the possibility of incrementally retraining the model using the validation data. Incremental training allows for gradual improvements, enabling the model to adapt as new patterns emerge, reflecting any shifts in customer behavior or market conditions.
...
The resulting RMSE and total costs will then be : 
...
kable(data_summary, format = "latex", booktabs = TRUE, linesep = "", format.args = list(big.mark = ",")) %>%
  kable_styling(font_size = 9, latex_options = c("hold_position")) %>%
  column_spec(2:ncol(data_summary), width = "2.5cm") %>%
  row_spec(0, bold = TRUE)
```

The RMSE indicates a relatively high level of error in the predictions, indicating that the estimated costs are significantly inflated compared to the actual values, particularly since many policies have no associated costs. Specifically, the estimated cost exceeds the actual value by nearly 1.5 million, which constitutes an increase of approximately 280%.
...
The resulting RMSE and total costs for the Linear Model will be : 
...
The RMSE is a litle bit lower, and the error is significantly lower with "only" a 56% overestimation compared to the average method.

Note that if we use more variables to predict the cost, we get a higher RMSE but a much lower error in total cost prediction with "only" an overestimation of 200,000 or 39%.

\newpage

## 6.4 Linear Regression

Substituting the optimal \( x^* \) back into \( RMSE(x) \):

\[
RMSE(x^*) = \sqrt{\frac{1}{N} \sum_{i=1}^{N} \left(\frac{1}{N} \sum_{j=1}^{N} y_j - y_i\right)^2}
\]

This value represents the minimum distance between \( x \) and the set of constants \( y_i \). The optimal solution is to set \( x \) equal to the mean of \( y_i \) to minimize the function.
...
This page is for the Final Test

\newpage

# 8. Conclusion

This page is for the Conclusion
